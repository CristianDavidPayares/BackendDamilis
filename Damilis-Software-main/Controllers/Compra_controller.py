...
def cntCreateCompra():
    ...
    if subtotal is None or not metodo_pago or not cli_id:
        return jsonify({"error": "subtotal, metodo_pago y cli_id son obligatorios"}), 400

    nueva_compra = addCompra(subtotal, metodo_pago, numero_compras, cli_id)
...
def cntUpdateCompra():
    ...
    filas_afectadas = upCompra(id, subtotal, metodo_pago, numero_compras)
...
def cntListComprasByCliente(cli_id):
